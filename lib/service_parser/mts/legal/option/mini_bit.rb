module ServiceParser
  class Mts
    class Legal::Option::MiniBit < ServiceParser::Mts
         ...
         :service_blocks => [
           {
             ...
             :row_item_text_to_split_by_regex => {
               ...
               /(?=(.*?первые.*последующие.*\s?\d+\s?мб))(\s?\d+\s?руб\.\s?).*?(\s?\d+\s?руб\.\s?).*/i => [1..1, 2..2], 
             },
             :param_name_tag => [],
             :param_value_tag => [],
           },
           {
             ...
             :row_item_text_to_split_by_regex =>  {
               ...
               /(?=(.*?первые.*последующие.*\s?\d+\s?мб))(\s?\d+\s?руб\.\s?).*?(\s?\d+\s?руб\.\s?).*/i => [1..1, 3..3], 
             },
             :param_name_tag => [],
             :param_value_tag => [],
           },
