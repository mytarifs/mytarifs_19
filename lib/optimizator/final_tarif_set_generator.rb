  ...
  #    raise(StandardError, options)
    ...
    class CurrentValueCalculator
      ...
      def tarif_set_for_service_that_depended_on(full_set_services)
        start = Time.now
        dependen_services = []; depended_on_services = []
        full_set_services.each do |full_set_service|
          dependen_services += ((services_that_depended_on[full_set_service] || []) & full_set_services) - dependen_services
        end
        
        (full_set_services - dependen_services).each do |full_set_service|
          depended_on_services << full_set_service if !((services_that_depended_on[full_set_service] || []) & dependen_services).blank?
        end
        
        return depended_on_services + dependen_services
        
        s0 = Time.now - start
        sliced_services_that_depended_on = services_that_depended_on.slice(*(full_set_services))
        s1 = Time.now - start - s0
        dependen_services = sliced_services_that_depended_on.values.flatten.uniq & full_set_services
        s2 = Time.now - start - s1 - s0
        depended_on_services = sliced_services_that_depended_on.map{|ds, ds_on| ds if true or !(ds_on & dependen_services).blank?}.compact.uniq
        s3 = Time.now - start - s1 - s2 - s0
        result = depended_on_services + dependen_services
        s4 = Time.now - start - s1 - s2 - s3 - s0
        raise(StandardError, [
          "#{[s0, s1, s2, s3, s4].map{|s| s*1000}}",
          "services_that_depended_on #{services_that_depended_on}",
          "full_set_services #{full_set_services}",
          "sliced_services_that_depended_on #{sliced_services_that_depended_on}",
          "dependen_services #{dependen_services}"
        ].join("\n")) if (Time.now - start) > 0.03
...
=end        
  ...
  #        result += (indexed_tarif_set[part_index][part_set_index] - result)
           result += indexed_tarif_set[part_index][part_set_index] #(indexed_tarif_set[part_index][part_set_index] - result)
        end
        result.join("_")#.join("/")
      end  
    end  
