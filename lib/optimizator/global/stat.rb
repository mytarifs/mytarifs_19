...
#    attr_reader :stat_params_by_global_category, :global_category_by_parts, :stat_params_grouped_by_period, :service_categories_by_global_category
    attr_reader :m_region_id, :operator_id
    ...
    def all_global_categories_stat(calls, service_ids = [])
      loaded_service_categories = service_categories_by_global_category(service_ids)
    ...
    def category_ids_for_global_category_stat(service_categories_by_global_category, global_category_filtr = nil, row = nil)
      ...
      service_categories_by_global_category.each do |sc|
        ...
        if global_category_filtr and sc['filtr']
          if Optimization::Global::Base::CategoryFiltr.new.filtr_for_stat(global_category_filtr, sc['filtr'], m_region_id, operator_id)
            result[true] << sc['id'] 
          else
            result[false] << sc['id']
          end
    ...
    def service_categories_by_global_category(service_ids = [])
      ...
      raise(StandardError) if m_region_id.blank?
      
      base_service_category = base_service_category_tarif_classes_with_m_region_filtr(service_ids)      
      base_service_category = base_service_category.where.not(:uniq_service_category => [nil, ''])
...
#      calls.select(fields).where(where_hash).group(group_by)#.group(group_period)
    ...
    def parts_from_calls(calls)
      all_part_sqls = []
      ::Optimization::Global::Base::StructureByParts.each do |part, structure_by_part|
        iterate(structure_by_part) do |params|
          all_part_sqls << calls.select("'#{part}' as part").where(call_filtr.filtr((params - [{}]))).having("count(id) > 0").to_sql
        end        
      end
      result = Customer::Call.find_by_sql(all_part_sqls.join(" union ")).uniq.map{|item| item.part}
      result      
    end
  ...
  #      day_group = stat_params_by_global_category['window_over'] == 'day' ? 'day' : nil
    ...
    def global_category_by_parts(service_ids = [])
      ...
      result = {}
      base = base_service_category_tarif_classes_with_m_region_filtr(service_ids) 
    ...
    def stat_params_by_global_category(service_ids = [], id = nil)
      ...
      base_service_category.
        joins(:service_category_tarif_classes, price_lists: [formulas: :standard_formula]).
        where.not(:service_category_tarif_classes => {:uniq_service_category => [nil, '']}).
    ...
    def base_service_category_tarif_classes_with_m_region_filtr(service_ids = [])
      base_service_category = Service::CategoryTarifClass.
        joins(as_standard_category_group: [price_lists: :formulas]).
        where("#{Service::CategoryTarifClass.regions_sql(m_region_id)}").
        where("#{Price::Formula.regions_sql(m_region_id)}")

      base_service_category = base_service_category.
        where(:service_category_groups => {:tarif_class_id => service_ids}) if !service_ids.blank?
