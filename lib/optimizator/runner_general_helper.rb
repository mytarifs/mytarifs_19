module Optimizator
  module RunnerGeneralHelper
    ...
    def clean_results(run_id, tarif_ids_to_clean = [])
      if run_id
        if tarif_ids_to_clean.blank?
          ...
          Result::Tarif.where(:run_id => run_id, :tarif_id => tarif_ids_to_clean).delete_all
          Result::ServiceSet.where(:run_id => run_id, :tarif_id => tarif_ids_to_clean).delete_all
          Result::Service.where(:run_id => run_id, :tarif_id => tarif_ids_to_clean).delete_all
          Result::Agregate.where(:run_id => run_id, :tarif_id => tarif_ids_to_clean).delete_all
          Result::ServiceCategory.where(:run_id => run_id, :tarif_id => tarif_ids_to_clean).delete_all
        end
      end
    end
    ...
    def prepare_options(options = {})
      ...
      selected_service_categories = !options[:selected_service_categories].blank? ? options[:selected_service_categories] :
        Optimization::Global::Base.new().global_names_by_parts(Optimization::Global::Base::StructureByParts.keys - excluded_service_category_parts)#['own_and_home_regions/internet', 'own_and_home_regions/calls_out/to_own_and_home_regions/to_operators']#, 'own_and_home_regions/sms_in', ]
      services_by_operator = options[:services_by_operator] || default_services
      temp_value = options[:temp_value] || {}
      raise(StandardError, [region_txt, privacy_id, options ]) if false
      {
       :optimization_params => {
          ...
          :calculate_one_operator_function_name => optimization_params['calculate_one_operator_function_name'] || 'calculate_tarif_cost_by_operator',
         },
       :user_input => {
...
#          :calculate_with_limited_scope => calculation_choices['calculate_with_limited_scope'],
          ...
          :user_region_id => Category::MobileRegions[region_txt].try(:[], 'region_ids'). try(:[], 0) || Category::Region::Const::Moskva,             
