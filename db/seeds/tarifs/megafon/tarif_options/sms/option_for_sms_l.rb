...
@tc.create_tarif_class({
  :id => _mgf_option_for_sms_l, :name => 'Опция для SMS L', :operator_id => Category::Operator::Const::Megafon, :privacy_id => _person, :standard_service_id => _special_service,
  ...
  scg_mgf_option_for_sms_l_sms = @tc.add_service_category_group(
    {:name => 'scg_mgf_option_for_sms_l_sms' }, 
    {:name => "price for scg_mgf_option_for_sms_l_sms"}, 
    {:calculation_order => 0, :standard_formula_id => Price::StandardFormula::Const::MaxCountVolumeForFixedPrice,  
      :formula => {:params => {:max_count_volume => 600.0, :price => 0.0}, :window_over => 'month' } }
    )

#Подключение услуги
  @tc.add_one_service_category_tarif_class(_sctcg_one_time_tarif_switch_on, {}, {:standard_formula_id => Price::StandardFormula::Const::PriceByItem, :formula => {:params => {:price => 300.0} } })
...
category = {:uniq_service_category => 'own_and_home_regions/sms_out/to_own_and_home_regions',}
  @tc.add_grouped_service_category_tarif_class(category, scg_mgf_option_for_sms_l_sms[:id])
...
category = {:uniq_service_category => 'own_country_regions/sms_out/to_own_and_home_regions',}
  @tc.add_grouped_service_category_tarif_class(category, scg_mgf_option_for_sms_l_sms[:id], :tarif_set_must_include_tarif_options => [_mgf_be_as_home] )  
